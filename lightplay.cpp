...
struct lightplay{
        ...
        //-------------------------------------------------------------
            ...
            void removefromheap(){for(int i=0;i<n;i++){delete []c[i];}delete []c;
                                return;
                                }

            void resetthesysduringcollision(){
                        double swc[n][2];
                        for(int i=0;i<n;i++){swc[i][0]=c[i][0];
                                             swc[i][1]=c[i][1];}
                        //coordiantes copied
                        removefromheap();
                        n=n+1;
                        assigntoheap();
                        for(int i=0;i<n-1;i++){c[i][0]=swc[i][0];
                                                c[i][1]=swc[i][1];}
                        c[n-1][0]=c[n-2][0];
                        c[n-1][1]=c[n-2][1];
            ...
            void ontarget(){Line lin(c[n-2][0],c[n-2][1],c[n-1][0],c[n-1][1]);
            ...
            bool on_mirror(){
                             for(int i=0;i<m+cubes*16;i++){
                                ...
                                double a1=sqrt((a-x1)*(a-x1)+(b-y1)*(b-y1)),b1=sqrt((a-x2)*(a-x2)+(b-y2)*(b-y2));
                ...
                bool on_sphere(){double a,b,x1,y1;
                                 a=c[n-1][0];
                                 b=c[n-1][1];
                             for(int i=0;i<s;i++){
                                  x1=S[i][0];
            ...
            lightplay(){cout<<"Spheres-space-Mirrors-space-Cubes-enter   ex: 1 1 1 then press enter"<<endl;
                        arrthecircles();
                        arrthemirrors();
                        arrthecubes();
                        n=2;
        ...
        if(sqrt((c[n-1][0]-tarc[0])*(c[n-1][0]-tarc[0])+(c[n-1][1]-tarc[1])*(c[n-1][1]-tarc[1]))<=50){ontarget();exit=1;cout<<"PASSED"<<endl;break;}
